...
std::unique_ptr<podio::ROOTWriter> createWriter(const std::string& outputFile, podio::EventStore& store) {
  return std::make_unique<podio::ROOTWriter>(outputFile, &store);
}
...
StatusCode PodioOutput::initialize() {
  ...
  m_writer = createWriter(m_filename, m_podioDataSvc->getProvider());
...
StatusCode PodioOutput::execute() {
  if (m_firstEvent) {
    m_firstEvent = false;
    for (const auto& collection : m_podioDataSvc->getCollections()) {
      debug() << "Processing collection " << collection.first << ": register for write? ";
      if (m_switch.isOn(collection.first)) {
        debug() << "yes";
        m_writer->registerForWrite(collection.first);
      }
      debug() << endmsg;
    }
  }
